...
class ZenohBus:
    ...
    # pubsub helpers for torch tensors
    
    def publish_torch_tensor(self, key: str, tensor: Any, *, encoding: Optional[str] =ENC_RAW) -> None:
        """
        Publish a PyTorch tensor as raw bytes.
        ...
        try:
            ...
            raise ImportError("PyTorch is not installed. Cannot publish torch tensor.")
        ...
        if arr.device.type != "cpu":
            arr = arr.cpu()
        payload = arr.numpy().tobytes()
        self.publish(key, payload, encoding=encoding)
        
        
    def subscribe_torch_tensor(self, key_expr: str, handler: Callable[[str, Any], None], dtype=torch.float32) -> zenoh.Subscriber:
        """
        Subscribe to a key expression and receive PyTorch tensors.
        """
        ...
        def _tensor_listener(key: str, payload: bytes) -> None:
            try:
                ...
                self._log.exception(f"Exception in tensor subscriber handler for '{key_expr}'")
