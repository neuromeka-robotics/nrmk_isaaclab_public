from neuromeka import IndyDCP3, control_msgs
import time
from sensor_drivers.utils.math_helpers import MathFunc
import numpy as np

# OpState: SYSTEM_OFF(0), SYSTEM_ON(1), 
# VIOLATE(2), COLLISION(8), 
# RECOVER_HARD(3), RECOVER_SOFT(4),
# IDLE(5), MOVING(6), TEACHING(7), STOP_AND_OFF(9),
# COMPLIANCE(10), BRAKE_CONTROL(11), 
# SYSTEM_RESET(12), SYSTEM_SWITCH(13), VIOLATE_HARD(15), MANUAL_RECOVER(16), TELE_OP(17)
class ROBOT_STATE:
    IDLE = 5
    MOVE = 6
    DIRECT_TEACHING = 7
    ...
    def in_failure_state(robot_state):
        return robot_state in [ROBOT_STATE.STOP,
                               ROBOT_STATE.VIOLATE,
    ...
    def in_recovery_state(robot_state):
        return robot_state in [ROBOT_STATE.RECOVER_HARD,
...
def compute_joint_target_with_ik(robot_client,
                                 target_ee_pos,
                                 collision_state,
                                 rl_joint_target_rad,
    ...
    # j_pos = np.array(robot_client.get_robot_data()["q"])
    ...
    # if True:
    # if not collision_state and pos_error < ee_error_threshold:
