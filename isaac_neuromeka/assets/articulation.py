...
class FiniteArticulation(Articulation):
    ...
    def reset(self, env_ids: Sequence[int] | slice | None = None):
        ...
        self._preprev_body_pos_w[env_ids] = 0
        self._prev_body_pos_w[env_ids] = 0
        self._finite_body_vel_w[env_ids] = 0
    ...
    def _create_buffers(self):
        ...
        self._preprev_body_pos_w = torch.zeros(self.num_instances, self.num_bodies, 3, device=self.device)
        self._prev_body_pos_w = torch.zeros(self.num_instances, self.num_bodies, 3, device=self.device)
        self._finite_body_vel_w = torch.zeros(self.num_instances, self.num_bodies, 6, device=self.device)
        
        # self.joint_target_history = HistoryBuffer(self.num_instances, self.num_joints, max_len=5, device=self.device)
        self.joint_pos_history = HistoryBuffer(self.num_instances, self.num_joints, max_len=self.history_len, device=self.device)
        self.joint_vel_history = HistoryBuffer(self.num_instances, self.num_joints, max_len=self.history_len, device=self.device)
    ...
    def update(self, dt: float):
        ...
        self._preprev_body_pos_w[:] = self._prev_body_pos_w[:]
        self._prev_body_pos_w[:] = self._data.body_state_w[:, :, :3]
        
        super().update(dt)
                
        self._finite_joint_vel[:] = (self._data.joint_pos - self._prev_joint_pos) / dt
        self._finite_body_vel_w[:, :, :3] = (self._data.body_state_w[:, :, :3] - self._prev_body_pos_w) / dt
