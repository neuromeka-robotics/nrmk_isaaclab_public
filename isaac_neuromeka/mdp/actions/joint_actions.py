...
if TYPE_CHECKING:
    from .action_cfgs import ResidualJointActionCfg, IKResidualActionCfg, ClampedJointActionCfg
...
class JointResidualAction(JointAction):
    ...
    def apply_actions(self, env_ids: Sequence[int] | None = None):
        ...
        self._asset.set_joint_position_target(self.joint_pos_target[env_ids], joint_ids=self._joint_ids, env_ids=env_ids)
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        if self._joint_ids is not None and self._joint_ids != slice(None):
            env_ids = env_ids.unsqueeze(-1)
        self._asset.set_joint_position_target(self.joint_pos_target[env_ids], joint_ids=self._joint_ids, env_ids=env_ids)  # TODO: fix required
...
class IKResidualAction(JointResidualAction):
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        if self._joint_ids is not None and self._joint_ids != slice(None):
            env_ids = env_ids.unsqueeze(-1)
        self._asset.set_joint_position_target(joint_pos, joint_ids=self._joint_ids, env_ids=env_ids)  # TODO: fix required
...
class JointVelocityAction(JointAction):
    ...
    def apply_actions(self, env_ids: Sequence[int] | None = None):
        ...
        self._asset.set_joint_velocity_target(self.joint_vel_target[env_ids], joint_ids=self._joint_ids, env_ids=env_ids)
